...
class Journals extends React.Component {
    ...
    render() {
        ...
        const cellList2 = ["제목", "날짜", "설정"]
        const filteredComponents = (data) => {
          data = data.filter((c) => {
            return c.cust_name.indexOf(this.state.searchKeyword) > -1;
          });
          return data.reverse().map((c) => {
            return <JournalList stateRefresh={this.stateRefresh} tableRow= {classes.tableRow} tableBody= {classes.tableBody} key={c.id} id={c.id} session_id={this.state.session_id} c={c} />
          });
        }
        return (
            <div>
        ...
        return (
            <TableRow className={this.props.tableRow}  id="row">
                ...
                <TableCell className={this.props.tableBody}>
                  ...
                  <JournalUpdate stateRefresh={this.props.stateRefresh} c={this.props.c}/>
