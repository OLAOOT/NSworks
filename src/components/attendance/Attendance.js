    ...
    render() {
        ...
        const filteredComponents = (data) => {
          ...
          return data.reverse().map((c) => {
            return <AttendanceList stateRefresh={this.stateRefresh} tableRow= {classes.tableRow} tableBody= {classes.tableBody} key={c.id} id={c.id} session_id={this.state.session_id} name={c.name} type={c.attendance_type} date={c.created_date} lat={c.latitude} lng={c.longitude} />
          });
        }
        return (
            <div>
              <Navigation current_link="/attendance"/>
